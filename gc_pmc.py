...
for k in range(len(ctlDates)):
	...
	ctlVals[k] = round(ctlY+(tssVals[k]-ctlY)/ctlDays,0)
	atlVals[k] = round(atlY+(tssVals[k]-atlY)/atlDays,0)
	ctlY = ctlVals[k]
	atlY = atlVals[k]
...
for k in range(len(ctlDates)):
	if ctlDates[k] >= startDate and ctlDates[k] <= endDate:
		...
		filteredATL.append(atlVals[k])
		filteredTSB.append(ctlVals[k]-atlVals[k])
